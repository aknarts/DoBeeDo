 ...
 */function b(i){return he({...i,state:!0,attribute:!1})}class f{constructor(e){this.connection=e}async getBoards(){return(await this.connection.sendMessagePromise({type:"dobeedo/get_boards"})).boards??[]}async getColumns(e){return(await this.connection.sendMessagePromise({type:"dobeedo/get_columns",board_id:e})).columns??[]}async createColumn(e,t,s){return(await this.connection.sendMessagePromise({type:"dobeedo/create_column",board_id:e,name:t,order_index:s})).column}async getTasks(e){return(await this.connection.sendMessagePromise({type:"dobeedo/get_tasks",board_id:e})).tasks??[]}async createTask(e,t,s,o){return(await this.connection.sendMessagePromise({type:"dobeedo/create_task",board_id:e,column_id:t,title:s,description:o})).task}async updateTask(e,t){const s={type:"dobeedo/update_task",task_id:e};return t.title!==void 0&&(s.title=t.title),t.description!==void 0&&(s.description=t.description),(await this.connection.sendMessagePromise(s)).task}async moveTask(e,t,s){const o={type:"dobeedo/move_task",task_id:e,target_column_id:t};return s!==void 0&&(o.target_sort_index=s),(await this.connection.sendMessagePromise(o)).task}async deleteTask(e){await this.connection.sendMessagePromise({type:"dobeedo/delete_task",task_id:e})}async deleteColumn(e){await this.connection.sendMessagePromise({type:"dobeedo/delete_column",column_id:e})}async deleteBoard(e){await this.connection.sendMessagePromise({type:"dobeedo/delete_board",board_id:e})}subscribeUpdates(e){const t=this.connection;if(t.subscribeEvents){console.debug("DoBeeDo: using connection.subscribeEvents for updates");const r=t.subscribeEvents(n=>{if(console.debug("DoBeeDo: raw WS message via subscribeEvents",n),typeof(n==null?void 0:n.event_type)=="string"&&n.data){const l=n.event_type;if(l.startsWith("dobeedo_")){const a=l.replace(/^dobeedo_/,"");e({event_type:a,payload:n.data,raw_type:l})}return}(n==null?void 0:n.type)==="dobeedo/event"&&n.event_type&&n.payload&&e({event_type:n.event_type,payload:n.payload,raw_type:n.raw_type})});return()=>{console.debug("DoBeeDo: unsubscribe from subscribeEvents"),r()}}if(!t.subscribeMessage)return console.warn("DoBeeDo: connection.subscribeMessage/subscribeEvents not available"),()=>{};const s=r=>{console.debug("DoBeeDo: raw WS message in subscribeUpdates handler",r),(r==null?void 0:r.type)==="dobeedo/event"&&r.event_type&&r.payload&&e({event_type:r.event_type,payload:r.payload,raw_type:r.raw_type})};console.debug("DoBeeDo: calling connection.subscribeMessage for subscribe_updates");const o=t.subscribeMessage(s,{type:"dobeedo/subscribe_updates"});return()=>{console.debug("DoBeeDo: unsubscribe from subscribe_updates"),o()}}async populateTestData(){await this.connection.sendMessagePromise({type:"dobeedo/populate_test_data"})}}var Ne=Object.defineProperty,Oe=Object.getOwnPropertyDescriptor,_=(i,e,t,s)=>{for(var o=s>1?void 0:s?Oe(e,t):e,r=i.length-1,n;r>=0;r--)(n=i[r])&&(o=(s?n(e,t,o):n(o))||o);return s&&o&&Ne(e,t,o),o};let u=class extends N{constructor(){super(...arguments),this._boards=[],this._tasks=[],this._columns=[],this._loading=!1,this._newTaskTitle="",this._newTaskDescription="",this._newColumnName="",this._unsubscribeUpdates=null,this._selectedBoardId=null,this._selectedColumnId=null,this._editingTaskId=null,this._editTaskTitle="",this._editTaskDescription="",this._movingTaskId=null}static get styles(){return _e`
    ...
    `}updated(i){if(i.has("hass")&&this.hass&&(this._fetchBoards(),!this._unsubscribeUpdates)){const e=new f(this.hass.connection);console.debug("DoBeeDo: registering subscribeUpdates"),this._unsubscribeUpdates=e.subscribeUpdates(t=>{var s,o;if(console.debug("DoBeeDo event received in panel",t),t.event_type.startsWith("task_")){const r=(s=t.payload.task)==null?void 0:s.board_id;console.debug("DoBeeDo task event for board",r,"selected",this._selectedBoardId),r&&r===this._selectedBoardId&&this._refreshTasksForSelectedBoard()}else if(t.event_type.startsWith("column_")){const r=(o=t.payload.column)==null?void 0:o.board_id;console.debug("DoBeeDo column event for board",r,"selected",this._selectedBoardId),r&&r===this._selectedBoardId&&this._refreshColumnsAndTasks()}else t.event_type.startsWith("board_")&&(console.debug("DoBeeDo board event"),this._fetchBoards())})}}async _fetchBoards(){if(this.hass){this._loading=!0;try{const i=new f(this.hass.connection);this._boards=await i.getBoards(),!this._selectedBoardId&&this._boards.length>0&&(this._selectedBoardId=this._boards[0].id),await this._refreshColumnsAndTasks()}catch(i){console.error("Failed to load DoBeeDo data",i)}finally{this._loading=!1}}}async _refreshColumnsAndTasks(){if(!this.hass||!this._selectedBoardId){this._columns=[],this._tasks=[],this._selectedColumnId=null;return}const i=new f(this.hass.connection);this._columns=await i.getColumns(this._selectedBoardId),this._tasks=await i.getTasks(this._selectedBoardId),this._columns.length===0?this._selectedColumnId=null:(!this._selectedColumnId||!this._columns.some(e=>e.id===this._selectedColumnId))&&(this._selectedColumnId=this._columns[0].id)}async _refreshTasksForSelectedBoard(){if(!this.hass||!this._selectedBoardId){this._tasks=[];return}const i=new f(this.hass.connection);this._tasks=await i.getTasks(this._selectedBoardId)}_handleSelectBoard(i){this._selectedBoardId!==i.id&&(this._selectedBoardId=i.id,this._refreshColumnsAndTasks())}async _handleCreateTask(){if(!this.hass||!this._selectedBoardId||!this._newTaskTitle.trim())return;const i=new f(this.hass.connection),e=this._boards.find(s=>s.id===this._selectedBoardId);if(!e)return;const t=this._selectedColumnId;if(!t){console.warn("No column selected on the selected board to create a task in.");return}try{const s=await i.createTask(e.id,t,this._newTaskTitle.trim(),this._newTaskDescription.trim()||void 0);this._newTaskTitle="",this._newTaskDescription="",e.id===this._selectedBoardId?this._tasks=[...this._tasks,s]:await this._refreshTasksForSelectedBoard()}catch(s){console.error("Failed to create DoBeeDo task",s)}}async _handleCreateColumn(){if(!this.hass||!this._selectedBoardId||!this._newColumnName.trim())return;const i=new f(this.hass.connection);try{const e=await i.createColumn(this._selectedBoardId,this._newColumnName.trim());this._newColumnName="",this._columns=[...this._columns,e]}catch(e){console.error("Failed to create DoBeeDo column",e)}}_startEditTask(i){this._editingTaskId=i.id,this._editTaskTitle=i.title,this._editTaskDescription=i.description??""}_cancelEditTask(){this._editingTaskId=null,this._editTaskTitle="",this._editTaskDescription=""}async _saveEditTask(){if(!this.hass||!this._editingTaskId)return;const i=this._tasks.find(r=>r.id===this._editingTaskId);if(!i)return;const e=this._editTaskTitle.trim();if(!e)return;const t={};e!==i.title&&(t.title=e);const s=this._editTaskDescription.trim();if(s!==(i.description??"")&&(t.description=s===""?null:s),!t.title&&t.description===void 0){this._cancelEditTask();return}const o=new f(this.hass.connection);try{const r=await o.updateTask(this._editingTaskId,t);this._tasks=this._tasks.map(n=>n.id===r.id?r:n),this._cancelEditTask()}catch(r){console.error("Failed to update DoBeeDo task",r)}}_startMoveTask(i){this._movingTaskId=i.id}_cancelMoveTask(){this._movingTaskId=null}async _handleMoveTask(i,e){if(!this.hass||e===i.column_id){this._cancelMoveTask();return}const t=new f(this.hass.connection);try{const s=await t.moveTask(i.id,e);this._tasks=this._tasks.map(o=>o.id===s.id?s:o),this._cancelMoveTask()}catch(s){console.error("Failed to move DoBeeDo task",s)}}async _handleDeleteTask(i){if(!this.hass||!window.confirm(`Delete task "${i.title}"?`))return;const e=new f(this.hass.connection);try{await e.deleteTask(i.id),this._tasks=this._tasks.filter(t=>t.id!==i.id)}catch(t){console.error("Failed to delete DoBeeDo task",t)}}async _handleDeleteColumn(i){if(!this.hass)return;const t=this._tasks.filter(r=>r.column_id===i.id).length,s=t>0?`Delete column "${i.name}" and ${t} task${t===1?"":"s"}?`:`Delete column "${i.name}"?`;if(!window.confirm(s))return;const o=new f(this.hass.connection);try{await o.deleteColumn(i.id),this._columns=this._columns.filter(r=>r.id!==i.id),this._tasks=this._tasks.filter(r=>r.column_id!==i.id)}catch(r){console.error("Failed to delete DoBeeDo column",r)}}async _handlePopulateTestData(){if(!this.hass||!window.confirm(`⚠️ WARNING: This will DELETE ALL existing boards, columns, and tasks!
...
Are you sure you want to continue?`))return;const i=new f(this.hass.connection);try{await i.populateTestData(),await this._fetchBoards()}catch(e){console.error("Failed to populate test data",e)}}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribeUpdates&&(this._unsubscribeUpdates(),this._unsubscribeUpdates=null)}render(){return h`
    ...
    `}};_([he({attribute:!1})],u.prototype,"hass",2),_([b()],u.prototype,"_boards",2),_([b()],u.prototype,"_tasks",2),_([b()],u.prototype,"_columns",2),_([b()],u.prototype,"_loading",2),_([b()],u.prototype,"_newTaskTitle",2),_([b()],u.prototype,"_newTaskDescription",2),_([b()],u.prototype,"_newColumnName",2),_([b()],u.prototype,"_unsubscribeUpdates",2),_([b()],u.prototype,"_selectedBoardId",2),_([b()],u.prototype,"_selectedColumnId",2),_([b()],u.prototype,"_editingTaskId",2),_([b()],u.prototype,"_editTaskTitle",2),_([b()],u.prototype,"_editTaskDescription",2),_([b()],u.prototype,"_movingTaskId",2),u=_([Ie("dobeedo-panel")],u)})();
